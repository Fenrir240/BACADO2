...
PDF_PATH = ROOT / "opere_pdf" / "enigma-otiliei.pdf"
ESSAY_PATH = ROOT / "modele_eseuri" / "enigma_otiliei.md"
OUTPUT_DIR = ROOT / "grafuri"
PROMPT_PATH = OUTPUT_DIR / "prompt-generare-graf-enigma-otiliei.md"
GRAPH_PATH = OUTPUT_DIR / "enigma-otiliei-knowledge-graph.json"
HTML_PATH = OUTPUT_DIR / "enigma-otiliei-knowledge-graph.html"
SCHEMA_PATH = OUTPUT_DIR / "enigma-otiliei-knowledge-graph.schema.json"
ONTOLOGY_PATH = OUTPUT_DIR / "enigma-otiliei-ontology.md"
RECONSTRUCTIONS_PATH = OUTPUT_DIR / "enigma-otiliei-chapter-reconstructions.md"
COVERAGE_PATH = OUTPUT_DIR / "enigma-otiliei-coverage-matrix.md"
VALIDATION_PATH = OUTPUT_DIR / "enigma-otiliei-validation-report.md"

CHAPTER_STARTS = (11, 31, 46, 62, 73, 83, 103, 120, 155, 174, 191, 214, 234, 255, 283, 307, 327, 365, 404, 446)
NOVEL_LAST_PAGE = 485
EVENTS_PER_CHAPTER = 8
...
def clean_page_text(raw: str, page_number: int) -> str:
    ...
    kept = []
    ignored = {str(page_number), "G. Călinescu", "Enigma Otiliei", "CUPRINS"}
    for line in lines:
        stripped = line.strip()
    ...
    for page_number in range(start, end + 1):
        ...
        for index, sentence in enumerate(sentences):
            if not 45 <= len(sentence) <= 720:
                continue
            passage = sentence
            if len(passage) < 180 and index + 1 < len(sentences):
                passage = f"{passage} {sentences[index + 1]}".strip()
...
def classify_concepts(passage: str) -> tuple[list[str], list[str]]:
    ...
    conflicts: list[str] = []
    rules = (
        ("THEME_INHERITANCE", ("mosten", "avere", "bani", "zestre", "testament", "adopt")),
        ("THEME_LOVE", ("iub", "sarut", "casator", "inima", "dragoste")),
        ("THEME_PATERNITY", ("tata", "copil", "tutore", "orfan", "fiica", "fiu")),
        ("THEME_FORMATION", ("facultat", "medicin", "doctor", "studi", "examen")),
        ("THEME_SOCIAL", ("famil", "societ", "lume", "bucurest", "capital")),
    )
    for concept_id, terms in rules:
        if any(term in text for term in terms):
            themes.append(concept_id)
    if any(term in text for term in ("mosten", "avere", "bani", "zestre", "testament", "fur")):
        conflicts.append("CONFLICT_SUCCESSION")
    if "otilia" in text and any(term in text for term in ("felix", "pascalopol", "iub", "casator")):
        conflicts.append("CONFLICT_LOVE")
    if "otilia" in text and any(term in text for term in ("liber", "viitor", "plec", "sacrific")):
        conflicts.append("CONFLICT_INNER")
    return themes or ["THEME_SOCIAL"], conflicts
...
def build_graph() -> dict[str, Any]:
    ...
    if len(reader.pages) < NOVEL_LAST_PAGE:
        raise ValueError("PDF-ul nu conține toate cele 20 de capitole așteptate.")
    page_texts = {
        number: clean_page_text(repair_pdf_encoding(reader.pages[number - 1].extract_text() or ""), number)
        for number in range(1, len(reader.pages) + 1)
    }
    essay = ESSAY_PATH.read_text(encoding="utf-8").strip()
    if "verosimilitatea" not in normalize(essay) or "tipologii umane" not in normalize(essay):
    ...
    for index, start in enumerate(CHAPTER_STARTS, start=1):
        end = CHAPTER_STARTS[index] - 1 if index < len(CHAPTER_STARTS) else NOVEL_LAST_PAGE
        chapter_ranges.append((index, start, end))

    sources: list[dict[str, Any]] = [
        {
            "id": "SRC_PDF",
            "source_type": "primary_text",
            "title": "Enigma Otiliei - text integral",
            "author": "George Călinescu",
            "path": "opere_pdf/enigma-otiliei.pdf",
            "sha256": sha256(PDF_PATH),
            "pdf_page_start": 11,
            "pdf_page_end": 485,
            "note": "Paginile PDF 486-500 conțin referințe istorico-literare și aparat critic, nu textul romanului.",
            "confidence": 1.0,
        },
        {
            "id": "SRC_ESSAY",
            "source_type": "model_essay",
            "title": "Eseu-model - Enigma Otiliei",
            "path": "modele_eseuri/enigma_otiliei.md",
            "sha256": sha256(ESSAY_PATH),
            "embedded_content": essay,
            "accepted_literary_traits": ["TRAIT_CHRONOTOPE", "TRAIT_TYPOLOGIES"],
            "confidence": 1.0,
        },
        {
            "id": "SRC_PROMPT",
            "source_type": "generation_specification",
            "title": "Promptul grafului 2 adaptat pentru Enigma Otiliei",
            "path": "grafuri/prompt-generare-graf-enigma-otiliei.md",
        ...
        sources.append(
            {
                ...
                "title": f"Enigma Otiliei - Capitolul {chapter_number}",
                "path": "opere_pdf/enigma-otiliei.pdf",
    ...
    nodes: list[dict[str, Any]] = [
        node(
            ...
            "Work",
            "Enigma Otiliei",
            "Roman obiectiv, citadin și realist-balzacian publicat în 1938, construit în jurul moștenirii lui Costache Giurgiuveanu și al formării lui Felix Sima.",
            importance="major",
            assertion_type="literary_interpretation",
            source_refs=["SRC_PDF", "SRC_ESSAY"],
            attributes={"publication_year": 1938, "genre": "epic", "species": "roman", "work_type": "roman", "chapter_count": 20, "literary_movement": "Realism balzacian"},
        ),
        node("AUTHOR_CALINESCU", "Author", "George Călinescu", "Scriitor, critic și istoric literar român, autorul romanului «Enigma Otiliei».", importance="major", source_refs=["SRC_PDF"], attributes={"full_name": "George Călinescu"}),
    ]
    ...
    for concept_id, concept_type, label, description in CONCEPTS:
        nodes.append(
            node(
                ...
                importance="major" if concept_id in {"THEME_INHERITANCE", "THEME_LOVE", "THEME_PATERNITY", "MOV_REALISM", "TRAIT_CHRONOTOPE", "TRAIT_TYPOLOGIES"} else "supporting",
                assertion_type="literary_interpretation",
                source_refs=["SRC_ESSAY", "SRC_PDF"],
                attributes={"accepted_for_essay": concept_id in {"TRAIT_CHRONOTOPE", "TRAIT_TYPOLOGIES"}},
            )
        )
    ...
    edges: list[dict[str, Any]] = [
        edge("EDGE_WORK_AUTHOR", "WORK_ENIGMA_OTILIEI", "authored_by", "AUTHOR_CALINESCU", "George Călinescu este autorul romanului.", ["SRC_PDF"]),
        edge("EDGE_WORK_MOVEMENT", "WORK_ENIGMA_OTILIEI", "belongs_to_literary_movement", "MOV_REALISM", "Romanul aparține realismului balzacian.", ["SRC_ESSAY"]),
        ...
        edge("EDGE_MOV_TRAIT_2", "MOV_REALISM", "has_characteristic", "TRAIT_TYPOLOGIES", "Eseul-model argumentează realizarea tipologiilor umane.", ["SRC_ESSAY"]),
        edge("EDGE_TRAIT_CHRONO_CITY", "TRAIT_CHRONOTOPE", "supported_by", "LOC_BUCURESTI", "Bucureștiul începutului de secol XX oferă cadrul spațial verificabil al acțiunii.", ["SRC_PDF", "SRC_ESSAY"]),
        edge("EDGE_TRAIT_CHRONO_ANTIM", "TRAIT_CHRONOTOPE", "supported_by", "LOC_ANTIM", "Strada Antim fixează precis cadrul incipitului și al finalului.", ["SRC_PDF", "SRC_ESSAY"]),
        edge("EDGE_TRAIT_TYPE_AVAR", "TRAIT_TYPOLOGIES", "supported_by", "CHAR_COSTACHE", "Costache Giurgiuveanu individualizează tipul avarului balzacian.", ["SRC_PDF", "SRC_ESSAY"]),
        edge("EDGE_TRAIT_TYPE_PARVENU", "TRAIT_TYPOLOGIES", "supported_by", "CHAR_STANICA", "Stănică Rațiu individualizează tipul parvenitului balzacian.", ["SRC_PDF", "SRC_ESSAY"]),
    ]
    ...
    for location_id, label, *_ in LOCATIONS:
        edges.append(
            edge(
                ...
                "has_setting",
                location_id,
                f"{label} este un spațiu al acțiunii romanului.",
                ["SRC_PDF"],
            )
        )
    for concept_id, *_ in CONCEPTS:
        if concept_id.startswith(("THEME_", "CONFLICT_", "MOTIF_", "TECH_")):
            edges.append(edge(f"EDGE_WORK_{concept_id}", "WORK_ENIGMA_OTILIEI", "has_concept", concept_id, "Concept relevant pentru construcția romanului.", ["SRC_PDF", "SRC_ESSAY"]))
    ...
    for chapter_number, start, end in chapter_ranges:
        ...
        selected = select_chapter_events(page_texts, chapter_number, start, end)
        event_ids: list[str] = []
        chapter_node = node(
            chapter_id, "Chapter", f"Capitolul {chapter_number}",
            f"Capitolul {chapter_number} al romanului, acoperit de paginile PDF {start}-{end}.",
        ...
        edges.append(edge(f"EDGE_WORK_CH_{chapter_number:02d}", "WORK_ENIGMA_OTILIEI", "contains", chapter_id, f"Romanul conține capitolul {chapter_number}.", [source_ref]))
        for chapter_order, item in enumerate(selected, start=1):
            ...
            event_node = node(
                ...
                attributes={
                    ...
                    "immediate_effects": (
                        ...
                        else ["Acțiunea încheie segmentul documentat al capitolului și pregătește continuarea romanului."]
                    ),
                    "long_term_effects": [],
                    "state_transitions": [],
                    "motivations": [],
            ...
            motif_id = None
            if any(term in normalized_passage for term in ("bani", "avere", "mosten", "testament", "saltea")):
                motif_id = "MOTIF_MONEY"
            elif any(term in normalized_passage for term in ("casa", "odaia", "salon", "strada antim")):
                motif_id = "MOTIF_HOUSE"
            elif "fotografi" in normalized_passage:
                motif_id = "MOTIF_PHOTO"
            if motif_id:
    ...
    for index in range(len(chapters) - 1):
        source_event = chapters[index]["event_sequence"][0]
    ...
    chapter_boundaries_valid = [
        (item["pdf_page_start"], item["pdf_page_end"])
        for item in chapters
    ] == [
        (start, CHAPTER_STARTS[index] - 1 if index < len(CHAPTER_STARTS) else NOVEL_LAST_PAGE)
        for index, start in enumerate(CHAPTER_STARTS, start=1)
    ]
    structural_valid = (
        not dangling
        ...
        and sum(item["type"] == "NarrativeState" for item in nodes) == 40
        and chapter_boundaries_valid
    )
    graph = {
        "metadata": {
            "title": "Knowledge graph narativ complet și autosuficient - Enigma Otiliei",
            "work": "Enigma Otiliei",
            "work_id": "enigma_otiliei",
            "author": "George Călinescu",
            "language": "ro",
            ...
            "work_type": "roman",
            "literary_movement": "Realism balzacian",
            "derivation": "Reconstruit după promptul exact al grafului 2 pentru Ion, adaptat la Enigma Otiliei și extins cu delimitarea capitolelor, curentul literar și elementele compoziționale; faptele provin din textul integral și eseul-model local.",
            "generation_prompt": "grafuri/prompt-generare-graf-enigma-otiliei.md",
        ...
        "ontology": {
            ...
            "accepted_literary_trait_ids": ["TRAIT_CHRONOTOPE", "TRAIT_TYPOLOGIES"],
        },
        "nodes": nodes,
        "edges": edges,
        "chapters": chapters,
        "reconstruction_profiles": [
            ...
            {"id": "bac_essay", "description": "Realism, două trăsături din eseul-model, tema iubirii, titlu și relația incipit-final.", "concept_ids": ["MOV_REALISM", "TRAIT_CHRONOTOPE", "TRAIT_TYPOLOGIES", "THEME_LOVE", "TECH_TITLE", "TECH_INCIPIT_FINAL"]},
        ],
        "validation": {
            ...
            "source_pdf_page_coverage": {"start": 11, "end": 485, "excluded_critical_appendix_start": 486},
            "model_essay_traits_locked": ["Verosimilitatea prin cronotop precis", "Tipologii umane balzaciene"],
        },
    }
...
def build_schema(graph: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "$id": "https://bacapp.local/schemas/enigma-otiliei-knowledge-graph.schema.json",
        "title": "Knowledge graph - Enigma Otiliei",
        "type": "object",
        ...
        "properties": {
            ...
            "chapters": {"type": "array", "minItems": 20, "maxItems": 20},
...
def build_coverage_markdown(graph: dict[str, Any]) -> str:
    ...
    edges = graph["edges"]
    lines = [
        "# Matrice de acoperire - Enigma Otiliei",
        "",
...
def build_validation_markdown(graph: dict[str, Any]) -> str:
    ...
    lines = [
        "# Raport de validare - Enigma Otiliei",
        "",
        ...
        f"- Delimitarea celor 20 de capitole: {'corectă' if validation['chapter_boundaries_valid'] else 'incorectă'}",
...
<body>
<header><div class="eyebrow">Graf narativ autosuficient</div><h1>{title}</h1><p>George Călinescu · text integral, 20 de capitole · fiecare eveniment păstrează citatul-sursă și pagina PDF.</p><div class="legend"><span><i class="dot" style="background:#7c2d3a"></i>Operă / capitole</span><span><i class="dot" style="background:#315b73"></i>Personaje</span><span><i class="dot" style="background:#3f725e"></i>Evenimente</span><span><i class="dot" style="background:#b68738"></i>Concepte</span></div></header>
<main>
